  ...
  try {
    const [
      ...
      prisma.$queryRaw<{ status: string; count: bigint }[]>`
        SELECT status, COUNT(*)::bigint AS count FROM news_posts GROUP BY status
      `,
    ...
    const trends: DashboardTrend[] = monthLabels.map((month) => ({
      label: month.label,
      value: Number(
    ...
    return new Response(JSON.stringify({ summary, trends, activities }), {
    ...
    return new Response(
      JSON.stringify({
        summary: { publishedNews: 0, draftNews: 0, totalResidents: 0, activeResidents: 0, latestResidents: 0 },
